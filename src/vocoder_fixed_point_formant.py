"""
vocoder_fixed_point_formant.py — Fixed-point vocoder with hardcoded formant-band coefficients.

Duplicate of vocoder_fixed_point.py with:
  - VOICE_BANDS updated to speech formant regions (fbands_formant defaults)
  - Filter coefficients hardcoded (no scipy at runtime for filter design)
  - COEF_BITS/COEF_FRAC unchanged at 16/14 — all new coefficients fit in Q2.14
...
Pipeline:
  ...
  4. Generate sawtooth: hw_int, apply same 3 bandpass filters
...
FS        = 24_000
ADC_BITS  = 8
DATA_FRAC = 7       # Q1.7: audio in (-1, 1), max fractional bits for 8-bit signed
COEF_BITS = 16
COEF_FRAC = 14      # Q2.14: covers a[] up to ±2, b[] down to ~6e-5 — fits all bands

SAW_FREQ  = 250     # Hz — updated to match notebook carrier_hz

# Formant-targeted bands (fbands_formant defaults):
#   Band 1: F1 vowel openness           300 –  800 Hz
#   Band 2: F2 vowel colour             800 – 2500 Hz
#   Band 3: F3 consonant character     2500 – 5000 Hz
VOICE_BANDS = [
    (800,  2500),
    (2500, 5000),
]
...
ENV_FC    = 300.0   # 8-bit constraint: (1-alpha)*weakest_band_peak must exceed 1 LSB
                    # band3 peak≈0.12 → need (1-alpha) ≥ 0.065 → ENV_FC ≥ ~270 Hz
ENV_ALPHA = float(np.exp(-2.0 * np.pi * ENV_FC / FS))  # ≈ 0.924


# ── Hardcoded SOS coefficients ────────────────────────────────────────────────
# Precomputed: sig.butter(1, band, btype='bandpass', output='sos', fs=48000)
# Format per row: [b0, b1, b2, a0, a1, a2]  (a0 always 1.0)
# All a[] within (-2, 2) and b[] > 6e-5 → Q2.14 (COEF_FRAC=14) is sufficient.
...
def _design_bandpass(flo: float, fhi: float) -> list[tuple[hw_int, hw_int]]:
    """Return quantised biquad (b, a) pairs from hardcoded SOS table."""
...
def process(audio_hw: hw_int,
    ...
    """
    Run the fixed-point vocoder chain on a Q1.7 audio signal and return the
    16-bit Q1.7 output. This is the bit-exact reference for vocoder.v.
    """
    import os
    ...
    acc = (products[0] + products[1]).truncate(ADC_BITS, DATA_FRAC)
    _dump(acc, '06_output')
    return acc


def load_audio(audio_path: str) -> hw_int:
    """Load a WAV file, downmix to mono, resample to FS, wrap as Q1.7 hw_int."""
